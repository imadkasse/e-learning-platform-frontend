    ...
    <div className="h-screen">
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-row-reverse justify-between gap-8 max-w-7xl mx-auto">
          ...
          <div className="flex-1 max-w-4xl">
            {/* Course Header */}
            <div className="bg-white rounded-2xl border-2 border-slate-300 overflow-hidden mb-8">
              <div className="p-8">
                <h1 className="apply-fonts-medium text-3xl text-slate-800 mb-6 leading-tight">
                  {course.title}
                </h1>
                ...
                <div className="flex items-center gap-4 p-6 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border-2 border-blue-200">
                  ...
                  <div className="flex flex-col gap-2">
                    <span className="text-blue-600 font-medium text-sm bg-blue-100 px-3 py-1 rounded-full w-fit border border-blue-200">
                      أستاذ الدورة
                    </span>
                    <h2 className="font-bold text-lg text-slate-800">
                      {instructor.username}
            ...
            <div className="bg-white rounded-2xl border-2 border-slate-300 overflow-hidden mb-8">
              {course?.sections?.[0]?.videos?.length > 0 ? (
                ...
                <div className="bg-white rounded-2xl border-2 border-slate-300 overflow-hidden mb-8">
                  <div className="p-6">
                    <div
                      className="relative w-full rounded-xl overflow-hidden bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center border-2 border-dashed border-slate-300"
                      style={{ paddingTop: "56.25%" }}
                    >
                      <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8">
                        {/* أيقونة رئيسية */}
                        <div className="w-16 h-16 bg-slate-200 rounded-full flex items-center justify-center mb-4 border-2 border-slate-300">
                          <Video className="w-8 h-8 text-slate-400" />
                        </div>
            ...
            <div className="bg-white rounded-2xl border-2 border-slate-300 overflow-hidden mb-8">
              <div className="p-8">
                ...
                <div className="bg-slate-50 rounded-xl p-6 border-r-4 border-blue-500 border-2 ">
                  <p className="apply-fonts-normal text-slate-700 leading-8 text-base">
                    {course.description}
                  </p>
                </div>
              ...
              <div className="p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {course.concepts?.map((concept, index) => (
                    <div
                      key={index}
                      className="flex items-center gap-3 p-4 bg-green-50 rounded-xl border-2 border-green-200 hover:border-green-300 hover:bg-green-100 transition-colors duration-300"
                    >
                      <CheckCircle
                        size={20}
                        className="text-green-600 flex-shrink-0"
                ...
                <div className="space-y-4">
                  {course.sections?.map((section, index) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => toggleSection(index)}
                        className="flex items-center justify-between w-full p-6 text-right hover:bg-slate-50 transition-all duration-300 group border-b-2 border-slate-300"
                      >
                        <div className="flex items-center gap-4">
                          <div className="transform transition-transform duration-300 group-hover:scale-110">
                            {openIndex === index ? (
                          ...
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-lg flex items-center justify-center text-white font-bold border-2 border-blue-300">
                              {index + 1}
                            </div>
                            <span
                              className={`text-lg font-bold ${
                      ...
                      {openIndex === index && (
                        <div className="border-t-2 border-slate-300 bg-slate-50">
                          {section.videos.map((video) => (
                            <div
                              key={video._id}
                              className="flex items-center py-4 px-6 hover:bg-white transition-colors duration-300 cursor-pointer group border-b border-slate-200 last:border-b-0"
                            >
                              <div className="flex items-center gap-4 flex-1">
                                <button className="flex items-center justify-center w-12 h-12 bg-gradient-to-br from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white rounded-full transition-all duration-300 transform hover:scale-110 border-2 border-blue-300">
                                  <Play size={16} className="mr-0.5" />
                                </button>
            ...
            <div className="bg-white rounded-2xl border-2 border-slate-300 overflow-hidden mb-8">
              <div className="p-8">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <div className="flex items-center gap-3 bg-gradient-to-r from-yellow-50 to-orange-50 px-4 py-3 rounded-xl border-2 border-yellow-300">
                    <span className="font-bold text-xl text-slate-800">
                      {course.avgRatings}
                    </span>
                    <Rating
                  ...
                  {course.reviews.length > 0 ? (
                    <div className="space-y-6">
                      {course.reviews.map((review) => (
                        <div
                          key={review._id}
                          className="bg-gradient-to-r from-slate-50 to-blue-50 rounded-2xl border-2 border-slate-300 hover:border-blue-300 transition-all duration-300 overflow-hidden"
                        >
                          <div className="flex items-center gap-4 p-6 border-b-2 border-slate-300">
                            <div className="relative">
                              <Image
                                src={
                                  review.user.thumbnail || "/imgs/logoImg.png"
                                ...
                                width={56}
                                height={56}
                                className="rounded-full border-3 border-blue-300"
                              />
                          ...
                          <div className="p-6">
                            <div className="bg-white rounded-xl p-6 border-r-4 border-blue-500 border-2 ">
                              <p className="apply-fonts-normal text-slate-700 leading-relaxed">
                                {review.content}
                              </p>
                            </div>
                    ...
                    <div className="text-center py-16 bg-gradient-to-br from-slate-50 to-blue-50 rounded-2xl border-2 border-dashed border-slate-400">
                      <div className="inline-flex items-center justify-center w-20 h-20 bg-slate-200 rounded-full mb-6 border-2 border-slate-300">
                        <svg
                          className="w-10 h-10 text-slate-400"
                          fill="none"
                          stroke="currentColor"
